...
In this document we will try to find the one, objectively finest imputation method by testing six of popular ones on forteen specially selected data sets prepared for these operations.

# Introduction + motivation

## Imputation functions

We'll look at the three simple imputation methods and three more advanced algorithms offered by popular packages.

TODO: czemu one; lanie wody o tym, że są fajne bo różne i badamy różne spojrzenia

1. *mode / median replace* - basic method which puts in a place of missing data median cells from their columns for numerical values and dominants for categorical ones.
...
4. *mice* ^1^ - method creating multiple imputations for multivariate missing data, based on Fully Conditional Specification, where each incomplete variable is imputed by a separate model. Standard imputation with *maxit* parameter of value 0 and default 5 number of imputed datasets
5. *VIM* ^2^ - standard k-nearest neighbors algorithm taken from the library
6. *missForest* ^3^ - simple imputation based on random forest offered by the package

## Datasets

TODO: ładniejszy format
...
TODO: tylko na sześciu wszystko się udało

## Binary classification algorithms

TODO: pięć metod, ich krótki opis i również krótkie stwierdzenie, że sa fajne bo różne i wgl.

# Description of methodology and results

## Reading datasets

In order to read all datasets we used another function, which returned a dataframe containing all the important informations about the test matrices.

TODO: trochę o metodzie, o funkcjach czytających; PARALLEL, itd
