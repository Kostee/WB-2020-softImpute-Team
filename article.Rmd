...
3. *'bogo' random replace* - simple algorithm replacing NA values with random numbers or factors from their columns.
...
6. *missForest* ^4^ - imputation based on random forest offered by the package.
...
The six imputations mentioned earlier were conducted on all fourteen of them, but only on six, imputations were successful in their entirety.
...
* *k-Nearest Neighbors from class^6^ package* - standard knn model attributing a given observation to a target corresponding to its closest observations in space,
...
* *random forest from ranger^8^* - algorithm consisting of many decisions trees uses bagging and feature randomness when building each individual tree trying to create an uncorrelated forest of trees whose prediction by committee is more accurate than that of any individual tree.
