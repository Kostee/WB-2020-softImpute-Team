...
3. *mice* - standard imputation with *maxit* parameter of value 0 and default 5 number of imputed datasets
4. *I4* - *vim* - default *kNN* implementation
5. *I5* - *missForest* - simple imputation offered by the package
...
In order to read all datasets we will use other function, which will return a dataframe containing all the important informations about the test matrices.
...
```{r}
### TU KOD HANI BEDZIE DO WKLEJENIA
...
The results of our work will be checked using the implemented classification tree algorithm from the *rpart* package. We have chosen this algorithm because this classifier works well for almost any type of data - and for the most part numerical, and for the most part factors, it can handle and make a good model. The use of an algorithm using, for example, the distance from the nearest points or separating sets by clusters would not be recommended for each data frame, and given the diversity, here rpart should do a good job.
