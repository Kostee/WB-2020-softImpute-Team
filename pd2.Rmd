...
imputation_times <- data.frame(rep(0, 14))
...
# żeby połączyć kolory i shape w legendzie zobacz
# https://stackoverflow.com/questions/12410908/combine-legends-for-color-and-shape-into-a-single-legend

ggplot(data=imptimes, aes(x = dataset_id, y = value, color = variable, shape = variable)) + geom_point() + 
#ggplot(data = imputation_times, aes(x = dataset_id, y )) +
  #geom_point(aes(y = remove), color = colors[1], size = 2, shape = 18) +
  #geom_point(aes(y = median), color = colors[2], size = 2, shape = 16) +
  #geom_point(aes(y = mice), color = colors[3], size = 2, shape = 15) +
  #geom_point(aes(y = missForest), color = colors[4], size = 2, shape = 17) +
  #geom_point(aes(y = VIM), color = colors[5], size = 2, shape = 19) +
